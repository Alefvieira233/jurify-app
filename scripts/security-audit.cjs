...
const headers = (vercelJson.headers || []).flatMap(item => item.headers || []);
...
function scanDir(dir, patterns) {
  ...
  for (const entry of entries) {
    ...
    } else if (entry.isFile() && /\.(ts|tsx|js)$/.test(entry.name) && relPath !== '/src/tests/setup.ts') {
      const content = fs.readFileSync(fullPath, 'utf8');
      for (const { pattern, label } of patterns) {
        if (pattern.test(content)) {
          found.push({ file: relPath, label });
        }
      }
    }
  }
