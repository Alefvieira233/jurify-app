...
// Check all header blocks, specifically looking for the wildcard /(.*) block
const allHeaders = vercelJson.headers || [];
const mainHeaderBlock = allHeaders.find(b => b.source === "/(.*)");
const headers = mainHeaderBlock?.headers || [];
...
function scanDir(dir, patterns) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory() && entry.name !== 'node_modules') {
      ...
      // Skip setup.ts and test files for secret scanning as they may contain dummy data
      if (entry.name === 'setup.ts' || entry.name.includes('.test.') || entry.name.includes('.spec.')) {
        continue;
      }
