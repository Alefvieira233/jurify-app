...
check(
  "console.log dropped in prod",
  viteConfig.includes("drop: isProd ? ['console'") ||
    viteConfig.includes('drop: isProd ? ["console"'),
);
check(
  "sourcemap set to hidden",
  viteConfig.includes("sourcemap: 'hidden'") ||
    viteConfig.includes('sourcemap: "hidden"'),
);
...
const headers = (vercelJson.headers || []).flatMap(
  (block) => block.headers || [],
);
...
check(
  ...
  headerKeys.includes("Content-Security-Policy"),
);
...
const dangerousPatterns = [
  ...
  {
    pattern: /eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9\.[a-zA-Z0-9._-]{50,}/,
    label: "Hardcoded JWT",
  },
];
