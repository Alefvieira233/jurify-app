...
const headers = vercelJson.headers ? vercelJson.headers.flatMap(h => h.headers || []) : [];
...
function scanDir(dir, patterns) {
  ...
  for (const entry of entries) {
    ...
    if (fullPath.endsWith(path.join('src', 'tests', 'setup.ts')) || fullPath.endsWith(path.join('src', 'tests', 'setup.js'))) continue;
