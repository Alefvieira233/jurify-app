...
// Scan all header blocks via .flatMap() to ensure comprehensive security header verification.
...
function scanDir(dir, patterns) {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory() && entry.name !== 'node_modules') {
      ...
      // Exclude setup.ts to avoid false positive hardcoded secret flags on dummy test JWT credentials
      if (entry.name.includes('setup.ts')) {
        continue;
      }
