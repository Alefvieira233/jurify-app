...
async function query(sql) {
  ...
  if (!r.ok) {
    if (r.status === 401) {
      console.warn(`\n⚠️  RLS coverage check SKIPPED: Supabase API error 401 Unauthorized.`);
      console.warn(`This usually happens when SUPABASE_ACCESS_TOKEN is missing or expired (e.g. on forks).\n`);
      process.exit(0);
    }
    throw new Error(
