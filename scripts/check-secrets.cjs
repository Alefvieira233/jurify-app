...
function walk(dir) {
  ...
  for (const entry of entries) {
    ...
    if (
      /\.(png|jpg|jpeg|gif|ico|svg|woff2?|ttf|eot|pdf|zip|lock)$/i.test(
        entry.name,
      )
    )
      continue;
    ...
    for (const p of PATTERNS) {
      ...
      while ((match = p.regex.exec(content)) !== null) {
        ...
        if (
          p.allowInTest &&
          /test-key|supabase-test|fake|example/i.test(snippet)
        )
          continue;
...
console.error("");
console.error(
  `Found ${issues.length} issue(s). Move values to env vars before committing.`,
);
process.exit(1);
