...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (earlyMethod && SERVICE_METHODS.includes(earlyMethod)) {
      // 🛡️ SECURITY: SERVICE-ROLE mode: caller is another internal edge function.
      // Explicitly verify service-role key to prevent unauthenticated external access
      // to administrative calendar methods.
      if (!isServiceRole(req)) {
        console.error(`[google-calendar] Unauthorized service method call: ${earlyMethod}`);
