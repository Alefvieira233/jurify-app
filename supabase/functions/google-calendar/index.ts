...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (earlyMethod && SERVICE_METHODS.includes(earlyMethod)) {
      ...
      // Verify caller actually has the service role key before proceeding.
      if (!isServiceRole(req)) {
        return new Response(
          JSON.stringify({ error: "Unauthorized: Service role key required for administrative methods" }),
