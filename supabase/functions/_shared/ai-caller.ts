...
export async function callOpenAI(
  ...
  if (params.persistLog !== false) {
    try {
      // PII redaction MUST happen BEFORE any character-based truncation
      // to avoid splitting sensitive tokens and bypassing regex matching.
      const redactedResult = redactPII(content);
      const preview = redactedResult.substring(0, 2000);
