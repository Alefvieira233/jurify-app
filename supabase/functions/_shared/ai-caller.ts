...
export async function callOpenAI(
  ...
  if (params.persistLog !== false) {
    try {
      // SECURITY: Redact PII from content BEFORE any truncation to avoid
      // splitting sensitive tokens and bypassing the redaction regex.
      const redacted = typeof content === "string" ? redactPII(content) : "";
      const preview = redacted.substring(0, 2000);
