...
export async function callOpenAI(
  ...
  if (params.persistLog !== false) {
    try {
      // Redact PII BEFORE truncation
      const resultRedacted = redactPII(content);
      ...
      await supabase.from("agent_ai_logs").insert({
        ...
        result_preview: resultRedacted.substring(0, 200),
        full_result: resultRedacted.substring(0, 2000),
        context: {
          source: params.source,
          latency_ms,
          ...(params.metadata ?? {}),
