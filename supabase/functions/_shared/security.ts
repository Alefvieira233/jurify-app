...
const PII_PATTERNS: Array<{ pattern: RegExp; label: string; replacement: string }> = [
  // Prioritize CNPJ over CPF to avoid partial matches on long sequences
  { pattern: /\b\d{2}\.?\d{3}\.?\d{3}\/?\d{4}-?\d{2}\b/g, label: "CNPJ", replacement: "***CNPJ***" },
  // Processo CNJ (Brazilian Lawsuit): NNNNNNN-DD.AAAA.J.TR.OOOO
  ...
  // OAB: OAB/UF 123456 or OAB 123456
  { pattern: /\b(?:OAB[\s\/\-]?)?(?:AC|AL|AP|AM|BA|CE|DF|ES|GO|MA|MT|MS|MG|PA|PB|PR|PE|PI|RJ|RN|RS|RO|RR|SC|SP|SE|TO)[\s\/\-]?\d{4,6}\b|\bOAB[\s\/\-]?\d{4,6}\b/gi, label: "OAB", replacement: "***OAB***" },
  { pattern: /\b\d{3}\.?\d{3}\.?\d{3}-?\d{2}\b/g, label: "CPF", replacement: "***CPF***" },
  // Brazilian Phone (handling international +55 prefix and optional parentheses)
  // Prioritize after CPF to allow exact 11-digit CPF matches to win over phone
