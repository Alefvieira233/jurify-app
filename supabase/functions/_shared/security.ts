...
const PII_PATTERNS: Array<{ pattern: RegExp; label: string; replacement: string }> = [
  ...
  { pattern: /\b\d{7}-\d{2}\.\d{4}\.\d\.\d{2}\.\d{4}\b/g, label: "PROCESSO_CNJ", replacement: "***PROCESSO***" },
  ...
  // OAB: UF + 4-6 digits (optional OAB prefix, case-insensitive)
  { pattern: /\b(?:OAB[/\s]?)?(?:AC|AL|AP|AM|BA|CE|DF|ES|GO|MA|MT|MS|MG|PA|PB|PR|PE|PI|RJ|RN|RS|RO|RR|SC|SP|SE|TO)[-\s/.]?[\d.]{4,9}\b/gi, label: "OAB", replacement: "***OAB***" },
