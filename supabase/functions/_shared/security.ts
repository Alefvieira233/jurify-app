...
const PII_PATTERNS: Array<{ pattern: RegExp; label: string; replacement: string }> = [
  ...
  { pattern: /(?:\+?55[\s.-]?)?\(?\d{2,3}\)?[\s.-]?(?:9\d{4}[\s.-]?\d{4}|[2-8]\d{3}[\s.-]?\d{4})\b/g, label: "Phone", replacement: "***PHONE***" },
  { pattern: /\b\d{2}\.?\d{3}\.?\d{3}-?[\dXx]\b/g, label: "RG", replacement: "***RG***" },
  { pattern: /\b(?:Bearer|JWT|Token)\s+[_a-zA-Z0-9.\-]{10,}\b/gi, label: "Token", replacement: "***TOKEN***" },
  { pattern: /\b(?:sk-|pk-|sbp|eyJ)[_a-zA-Z0-9.\-]{10,}\b/g, label: "Token", replacement: "***TOKEN***" },
];
