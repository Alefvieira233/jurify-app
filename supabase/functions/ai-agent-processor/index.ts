...
async function logAIProcessing(
  ...
  try {
    ...
    const systemPromptRedacted = redactPII(request.systemPrompt);
    const userPromptRedacted = redactPII(request.userPrompt);
    const fullResultRedacted = redactPII(response.result);
