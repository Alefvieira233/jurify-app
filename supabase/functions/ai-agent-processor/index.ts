...
async function logAIProcessing(
  ...
  try {
    ...
    await supabase.from("agent_ai_logs").insert({
      ...
      // Advanced Logging (LangSmith Style) — truncated and redacted to protect PII
      ...
      context: request.context ? JSON.parse(redactPII(JSON.stringify(request.context))) : null,
