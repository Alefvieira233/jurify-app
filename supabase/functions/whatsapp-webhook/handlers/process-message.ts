...
export async function processNormalizedMessage(
  ...
  try {
    ...
    if (!tenantId) {
      ...
      // Redact full text before truncation to avoid splitting sensitive tokens.
    ...
    try {
      ...
      if (executionRowId) {
        // P0 SECURITY: redact PII from results and prompts before logging.
        // Apply redaction to the full string before truncation.
