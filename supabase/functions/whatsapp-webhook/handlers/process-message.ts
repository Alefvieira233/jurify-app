...
export async function processNormalizedMessage(
  ...
  try {
    ...
    const executionId = `exec_${Date.now()}_${array[0]!.toString(36).substring(0, 9)}`;
    ...
    if (!budgetExceeded) try {
      ...
      if (executionRowId) {
        ...
        void supabase.from("agent_ai_logs").insert({
          ...
          // Advanced Logging (LangSmith Style) — truncated AFTER redaction to prevent PII leaks at boundaries
