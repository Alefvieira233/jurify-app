...
export async function processNormalizedMessage(
  ...
  try {
    ...
    if (!tenantId) {
      ...
      // SEC-01: Redact BEFORE truncating to avoid split PII patterns in logs
      const safeLogText = redactPII(text).substring(0, 80);
      console.error(`[processMsg:${provider}] TENANT_RESOLUTION_FAILED_STRICT | from=${from} | instance=${instanceName} | type=${messageType} | text="${safeLogText}"`);
