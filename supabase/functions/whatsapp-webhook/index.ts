      ...
      if (isKapsoPayload(firstPayload) || isBatch) {
        ...
        if (event === "messages.upsert" || event === "whatsapp.message.received") {
          ...
          if (normalized) {
            ...
            const redactedTextPreview = redactPII(normalized.text).substring(0, 50);
            console.log(`[webhook:kapso] Processing message from ${normalized.from}: "${redactedTextPreview}"`);
