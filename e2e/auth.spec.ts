...
test.describe('Jurify — Autenticação', () => {
  ...
  test('deve bloquear cadastro com senha fraca', async ({ page }) => {
    ...
    await page.locator('input[name="password"]').fill('fraca');
    await page.locator('input[name="confirmPassword"]').fill('fraca');
    await page.getByRole('button', { name: /começar agora/i }).click();

    // Check strength indicator or error message
    await expect(page.getByText(/senha fraca|mínimo 8 caracteres/i).first()).toBeVisible({ timeout: 5_000 });
  });
