/** Manages automated follow-up sequences with step configuration and activation. */
...
export function useFollowUpSequences() {
  ...
  const toggleActive = useCallback(async (id: string, currentActive: boolean) => {
    ...
    } catch (err) { log.error('toggleActive failed', err); }
  }, [updateMutation]);
  ...
  const createSequence = useCallback(async (input: CreateSequenceInput): Promise<boolean> => {
    ...
    try { await createMutation.mutateAsync(input); return true; } catch (err) { log.error('createSequence failed', err); return false; }
  }, [user, createMutation]);

  const updateSequence = useCallback(async (id: string, data: UpdateSequenceInput): Promise<boolean> => {
    try { await updateMutation.mutateAsync({ id, data }); return true; } catch (err) { log.error('updateSequence failed', err); return false; }
  }, [updateMutation]);

  const deleteSequence = useCallback(async (id: string): Promise<boolean> => {
    try { await deleteMutation.mutateAsync(id); return true; } catch (err) { log.error('deleteSequence failed', err); return false; }
  }, [deleteMutation]);
