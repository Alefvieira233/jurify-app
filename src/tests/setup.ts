...
// Fallback if env vars are empty or set to .env.example placeholder values
const dummyUrl = 'https://test.supabase.co';
const dummyKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZS10ZXN0Iiwicm9sZSI6ImFub24iLCJleHAiOjk5OTk5OTk5OTl9.test-key';

if (!import.meta.env.VITE_SUPABASE_URL || import.meta.env.VITE_SUPABASE_URL === 'your_supabase_url') {
  (import.meta.env as Record<string, string>).VITE_SUPABASE_URL = dummyUrl;
}
if (!import.meta.env.VITE_SUPABASE_ANON_KEY || import.meta.env.VITE_SUPABASE_ANON_KEY === 'your_supabase_anon_key') {
  (import.meta.env as Record<string, string>).VITE_SUPABASE_ANON_KEY = dummyKey;
}
