...
describe("sanitizeInput — homoglyph attack resistance", () => {
  ...
  it('blocks non-adjacent/flexible prompt injection: "ignore all previous prompts"', () => {
...
describe("redactPII — CPF", () => {
  ...
  it("redacts CPF embedded in a sentence", () => {
    expect(redactPII("Cliente 111.222.333-44 solicitou extrato")).toBe(
      "Cliente ***CPF*** solicitou extrato",
    );
  });
});
...
describe("redactPII — Expanded prioritized Brazilian patterns", () => {
  it("redacts CNPJ", () => {
    expect(redactPII("CNPJ do parceiro: 12.345.678/0001-90")).toBe(
      "CNPJ do parceiro: ***CNPJ***",
    );
    expect(redactPII("CNPJ: 12345678000190")).toBe("CNPJ: ***CNPJ***");
  });

  it("redacts Processo CNJ", () => {
    expect(redactPII("Processo número 0001234-56.2023.8.26.0100")).toBe(
      "Processo número ***PROCESSO***",
    );
  });

  it("redacts Email", () => {
    expect(redactPII("Contato por joao.silva@empresa.com.br")).toBe(
      "Contato por ***EMAIL***",
    );
  });

  it("redacts OAB", () => {
    expect(redactPII("Advogado OAB SP123456")).toBe("Advogado OAB ***OAB***");
    expect(redactPII("Advogada OAB/MG 654321")).toBe("Advogada OAB/***OAB***");
  });

  it("redacts Tokens (Bearer, sbp, JWT, sk-...)", () => {
    expect(
      redactPII("Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.abc.def"),
    ).toBe("***TOKEN***");
    expect(redactPII("sk-abcdef1234567890abcdef123")).toBe("***TOKEN***");
    expect(redactPII("eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.abc.def")).toBe(
      "***TOKEN***",
    );
  });

  it("redacts Phone and resolves CPF priority overlap", () => {
    expect(redactPII("Meu telefone é 11999998888")).toBe(
      "Meu telefone é ***PHONE***",
    );
    expect(redactPII("Telefone: (11) 98888-7777")).toBe(
      "Telefone: ***PHONE***",
    );
    expect(redactPII("+55 11 97777-6666")).toBe("***PHONE***");
