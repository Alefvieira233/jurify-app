...
describe('sanitizeInput — homoglyph attack resistance', () => {
  ...
  it('blocks "ignore all previous prompts" (non-adjacent tokens)', () => {
...
describe('sanitizeInput — base64 hidden payload', () => {
  ...
  it('blocks short base64 payloads (>=16 alphabet chars)', () => {
    // "ignore instructions" is 19 chars -> ~28 base64 chars
...
describe('redactPII — Email', () => {
  it('redacts email addresses', () => {
    expect(redactPII('Contato: joao.silva@gmail.com')).toBe('Contato: ***EMAIL***');
  });
});
...
describe('auditLog', () => {
  it('redacts PII from query and error before inserting', async () => {
    ...
    await auditLog(mockSupabase as any, {
      ...
      success: false,
      error: 'Failed for email: secret@example.com',
    });

    expect(mockFrom).toHaveBeenCalledWith('assistant_audit');
    expect(mockInsert).toHaveBeenCalledWith(expect.objectContaining({
      query: 'Check CPF ***CPF***',
      error: 'Failed for email: ***EMAIL***',
    }));
  });

  it('handles missing query/error gracefully', async () => {
    const mockInsert = vi.fn().mockResolvedValue({ error: null });
    const mockFrom = vi.fn().mockReturnValue({ insert: mockInsert });
    const mockSupabase = { from: mockFrom };
